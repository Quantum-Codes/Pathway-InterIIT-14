...
# class LLMExplanation(pw.Schema):
#     risk_level: str
#     short_reason: str
#     long_reason: str
...
# class ScoreWithExplanationResponse(pw.Schema):
...
def call_xpack_llm(features: Dict[str, Any], scores: Dict[str, float]) -> Dict[str, Any]:
    ...
    try:
        ...
        #     file.write(f"Model raw table dicts: {tbl_dicts}\n")
        #     file.flush()
        ...
        #     file.write(f"Parsed explanation: {json.dumps(explanation)}\n")
        #     file.flush()
        ...
        # with open("debug.txt", "a") as file:
        #     file.write(f"Error in LLM call: {str(e)}\n")
        #     file.flush()
...
transactions = pw.io.csv.read(
    path="/home/hades/Pathway/streaming-20251111T181915Z-1-001/streaming/inbox/transcations.csv",
...
rps_output = result_dict.select(
    user_id=pw.this.result["user_id"],
    full_name=pw.this.result["full_name"],
    p_ml=pw.this.result["p_ml"],
    anomaly=pw.this.result["anomaly"],
    evidence=pw.this.result["evidence"],
    rps=pw.this.result["rps"],
    risk_level=pw.this.result["risk_level"],
    short_reason=pw.this.result["short_reason"],
    long_reason=pw.this.result["long_reason"],
    recommended_action=pw.this.result["recommended_action"],
    tags=pw.this.result["tags"],
)

pw.io.jsonlines.write(rps_output, "/home/hades/Pathway/streaming-20251111T181915Z-1-001/streaming/inbox/rps_output.jsonl")

# import pandas as pd
# df = pd.read_csv("rps_output.csv")
# # Explicitly convert numeric columns
# df['p_ml'] = pd.to_numeric(df['p_ml'], errors='coerce')
# df['anomaly'] = pd.to_numeric(df['anomaly'], errors='coerce')
# df['evidence'] = pd.to_numeric(df['evidence'], errors='coerce')
# df['rps'] = pd.to_numeric(df['rps'], errors='coerce')
# df.to_csv('/home/hades/Pathway/streaming-20251111T181915Z-1-001/streaming/inbox/rps_output.csv', index=False)
...
# app = FastAPI(title="Fraud Explainer Service", version="1.0.0")

# @app.post("/score_raw", response_model=ScoreResponse)
# def score_raw(req: FeatureRequest):
#     """
#     Just proxy to existing /score.
#     """
#     scores = call_score_service(req.features)
#     return scores


# @app.post("/score_llm", response_model=ScoreWithExplanationResponse)
# def score_with_llm(req: FeatureRequest):
#     """
#     Call existing /score AND Ollama LLM explainer.
#     Returns both raw scores and a human-friendly explanation.
#     """
    
#     scores = call_score_service(req.features)

    
#     explanation_dict = call_xpack_llm(req.features, scores)
    
#     return {
#         "features": req.features,
#         "scores": scores,
#         "llm_explanation": explanation_dict,
#     }


# @app.get("/")
# def root():
#     return {"status": "ok", "message": "Fraud explainer service is running."}
