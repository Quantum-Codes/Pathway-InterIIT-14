...
load_dotenv()
pw.set_license_key(os.environ["PW_LICENSE"])
...
def ofac_screen(name: str):
    ...
    API_KEY = os.environ["SANCTIONS_API_KEY"]
    cases = [{"name": name}]
    resp = requests.post(
        f"{BASE_URL}/screen",
...
class ApiSelector(McpServable):
    def ofac_call(self, input_table: pw.Table) -> pw.Table:
        """Check if `name:str` is on OFAC sanctions list."""
    ...
    def news_call(self, input_table: pw.Table) -> pw.Table:
        """Search adverse media or news about `name:str`"""
